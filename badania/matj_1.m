function out = maij_1(action, varargin)
% MAIJ_1
...
%           'headless', true);
%   df = maij_1('simulate','scenario','impas','years',3,'seed',12345);
%   m  = maij_1('compute_metrics', df, 3, ...
%           'stab_crit',0.30,'elite_crit',0.35,'elite_streak_weeks',4);
%   mc = maij_1('monte_carlo','scenario','impas','years',3,'n',100,'spread',0.20,'seed',42);
%   sp = maij_1('spec');
%
%   r = maij_1('test', ...
%       'save_figures', false, ...
...
%   type(fullfile(r.out_dir,'report.txt'))
    ...
    switch action
        ...
        case "compute_extra_metrics"
            if nargin < 2
                error('maij_1:compute_extra_metrics', 'Podaj tabelę symulacji.');
            end
        ...
        otherwise
            error('maij_1:unknownAction', 'Nieznana akcja: %s', action);
    end
end
...
function out = action_transform_unit_samples(varargin)
    if nargin < 1
        error('maij_1:transform', 'Podaj macierz U z [0,1].');
    end
    U = varargin{1};
...
function out = action_aggregate_replicates(varargin)
    if nargin < 1
        error('maij_1:aggregate_replicates', 'Podaj tablicę wyników replik.');
    end
...
function report = test_suite(years, seed, out_root, python_outputs_dir, run_sobol, sobol_N, monte_carlo_n, mc_spread, mc_use_python_rng, headless, save_figures, save_tables, save_reports, save_workspace, include_python_compare)
    ...
    if save_workspace
        save(fullfile(out_dir, 'maij_1_workspace.mat'), 'report');
    end

    fprintf('=============================================================\n');
    fprintf('MAIJ_1 SUITE\n');
    fprintf('=============================================================\n');
    fprintf('out_dir        : %s\n', out_dir);
...
function save_scenario_fig(results, out_dir, headless)
    ...
    f = figure('Visible',vis,'Color','w','Name','maij_1_scenarios');
    tiledlayout(3,2);
    ...
    exportgraphics(f, fullfile(out_dir, 'maij_1_scenarios.png'), 'Resolution', 150);
    close(f);
end

function save_mc_fig(mc, out_dir, headless)
    vis = ternary(~headless, 'on', 'off');
    f = figure('Visible',vis,'Color','w','Name','maij_1_mc');
    tiledlayout(2,2);
    ...
    exportgraphics(f, fullfile(out_dir, 'maij_1_monte_carlo.png'), 'Resolution', 150);
    close(f);
end
